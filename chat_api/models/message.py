...
class Message(models.Model):
    ...
    request_id = models.OneToOneField(MessageRequest,on_delete=models.CASCADE,
        blank=True,null=True,related_name="message"
    )
    response_id = models.OneToOneField(MessageResponse,on_delete=models.CASCADE,
        blank=True,null=True,related_name="message"
    )
    output_id = models.OneToOneField(MessageOutput,on_delete=models.CASCADE,
        blank=True,null=True,related_name="message"
    )
    ...
    # Matches app schema
    metadata = models.JSONField(blank=True,null=True,validators=[      validate_metadata_size]    )  
    # Matches app schema
    has_embedding = models.BooleanField(default=False)
    has_document = models.BooleanField(default=False) 
    # Represents whether the message has an associated document 
    doc_url = models.FileField(upload_to="message_docs/",blank=True,
        null=True,validators=[validate_document_size])
    vote = models.BooleanField(default=False, blank=True, null=True) 
    # Matches app schema
    def __str__(self):
